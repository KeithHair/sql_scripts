...
DECLARE @ShowConstraints varchar(3)='' -- yes or no
...
SELECT 
	...
	CASE WHEN @ShowConstraints = 'yes' THEN	u.CONSTRAINT_SCHEMA+'.'+u.CONSTRAINT_NAME
	ELSE '----'
	END AS 'ConstraintName'
FROM INFORMATION_SCHEMA.COLUMNS c
JOIN INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE u
ON (c.TABLE_CATALOG+'.'+c.TABLE_SCHEMA+'.'+c.TABLE_NAME) = (u.TABLE_CATALOG+'.'+u.TABLE_SCHEMA+'.'+u.TABLE_NAME)
...
WHERE
	...
	AND
	(u.CONSTRAINT_NAME LIKE '%'+@FindConstraintLike+'%')
